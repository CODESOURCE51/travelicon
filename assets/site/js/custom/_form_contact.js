jQuery(document).ready(function() {
    ...
    jQuery('form.contact_1', '.contact_form_1').on('submit', function( e ){
        if(jQuery(this).attr('data-valid')!='1') e.preventDefault();
		...
		if( !emailReg.test(emailaddressVal) ){
            ...
            jQuery($form).attr('data-valid',0).find('div.result', '.contact_form_1').attr('disabled', false).append('<p class="error_item">invalid email address</p>');
        };

        if ($form.find('#contact_form_username', '.contact_form_1').hasClass('error_fields_class')) {
            jQuery($form).attr('data-valid',0).find('div.result', '.contact_form_1').attr('disabled', false).append('<p class="error_item">The name can\'t be empty</p>');
        };
        if ($form.find('#contact_form_email', '.contact_form_1').hasClass('error_fields_class')) {
			if ($form.find('div.result', '.contact_form_1').hasClass('error_email_mask')) {} 
	        	...
	        	jQuery($form).attr('data-valid',0).find('div.result', '.contact_form_1').attr('disabled', false).append('<p class="error_item">Too short (or empty) email address</p>');
	        }
        };

        if ($form.find('#contact_form_subj', '.contact_form_1').hasClass('error_fields_class')) {
            jQuery($form).attr('data-valid',0).find('div.result', '.contact_form_1').attr('disabled', false).append('<p class="error_item">The subject can\'t be empty</p>');
        };
        if ($form.find('#contact_form_message', '.contact_form_1').hasClass('error_fields_class')) {
            jQuery($form).attr('data-valid',0).find('div.result', '.contact_form_1').attr('disabled', false).append('<p class="error_item">The message text can\'t be empty</p>');
        };
		...
		if($form.attr('data-valid') !='1' ) $form.submit();
    });
});